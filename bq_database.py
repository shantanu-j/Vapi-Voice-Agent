...
from settings import PROJECT_ID, DATASET_ID, APPOINTMENTS_TABLE_ID, COMPLAINTS_TABLE_ID
...
def find_existing_appointment(
    ...
    """
    Finds an active appointment for the same phone number and start time.
    Used to make schedule_appointment idempotent so that if the
    request is retried after a connection error, we don't create a duplicate appointment/calendar event.
    """
...
def find_appointments_for_day(phone_number: str, date_: dt.date | None = None, only_active: bool = True) -> list[dict]:
    """
    Used by the cancel-lookup step. If date_ is given, returns appointments for
    phone_number on that calendar day (with a 1-day buffer either side, to absorb
    timezone drift). If date_ is None, returns all matching appointments for
    phone_number.
    """
    conditions = ["phone_number = @phone_number"]
    params = [bigquery.ScalarQueryParameter("phone_number", "STRING", phone_number)]
    ...
    query = f"""
        SELECT appointment_id, phone_number, name, reason, start_time, canceled, calendar_event_id, meet_link, timezone, created_at
        FROM `{APPOINTMENTS_TABLE_REF}`
...
def find_appointments_between(start_dt_utc: dt.datetime, end_dt_utc: dt.datetime) -> list[dict]:
    """
    Returns all active appointments whose start_time falls in [start_dt_utc, end_dt_utc).
    Used by available_slots to find what's already booked in a search window.
    """
    query = f"""
...
def cancel_appointment_by_id(appointment_id: str, phone_number: str, cancellation_reason: str | None = None) -> int:
    """Cancels exactly one appointment by its ID. Returns 1 if canceled, 0 if not found / already canceled."""
    query = f"""
...
def find_conflicting_appointments(
    ...
    """
    Returns any active appointments that overlap the requested [start_time, end_time] window.
    """
...
def reschedule_appointment_by_id(
    ...
    """
    Moves an active appointment to a new start_time, in place — same appointment_id,
    same calendar_event_id, same meet_link. Returns 1 if updated, 0 if not found /
    already canceled / phone number mismatch.
    """
    query = f"""
        UPDATE `{APPOINTMENTS_TABLE_REF}`
